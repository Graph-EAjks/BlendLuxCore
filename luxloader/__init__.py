...
if os.path.exists(install_offline_folder):
    ...
    files_filtered = [
        f for f in files[2] if f.startswith("pyluxcore") and f.endswith(".whl")
    ]
...
def _update_manifest():
    ...
    manifest_path = (
        pathlib.Path(__file__).parent.resolve() / "blender_manifest.toml"
    )
    files, *_ = os.walk(wheel_dl_folder)
    wheels = [pathlib.Path(wheel_dl_folder, f).as_posix() for f in files[2]]
...
def _get_installation_info():
    # Check if the versio to be installed matches the descritption saved during
    # the last install.
...
def _check_offline_content():
    ...
    if len(files) == 0:
        raise RuntimeError(
            "BlendLuxCore Installation Error: The install_offline/ directory exists but is empty!"
        )
    for file in files:
    ...
    if blc_wheel_path:
        wheel_hash = (
            base64.urlsafe_b64encode(
                hashlib.sha256(open(blc_wheel_path, "rb").read()).digest()
            )
            .decode("latin1")
            .rstrip("=")
        )
